...
class AuditControllerTest {
        ...
        @BeforeEach
        void setUp() {
                AuditController controller = new AuditController(auditService, auditEmitterRegistry);
        ...
        @Test
        void streamAudit_registersEmitter_andDoesNotPush_whenAuditStillProcessing() throws Exception {
                UUID auditId = UUID.randomUUID();
                AuditResponse inProgress = sampleAuditResponse(auditId);
                when(auditService.getAuditById(eq(auditId), eq(principal))).thenReturn(inProgress);
                ...
                mockMvc.perform(get("/api/v1/audits/{id}/stream", auditId))
                                .andExpect(request().asyncStarted())
                                .andExpect(status().isOk());

                verify(auditEmitterRegistry).register(auditId);
                verify(auditEmitterRegistry, org.mockito.Mockito.never()).pushAndComplete(any(), any());
        }
        ...
        void streamAudit_pushesImmediately_whenAuditAlreadyComplete() throws Exception {
                ...
                verify(auditEmitterRegistry).pushAndComplete(auditId, completed);
        }

        @Test
        void streamAudit_returns404_whenAuditNotFoundOrNotOwned() throws Exception {
                UUID auditId = UUID.randomUUID();
                when(auditService.getAuditById(eq(auditId), eq(principal)))
                                .thenThrow(new ApiException("Audit not found", HttpStatus.NOT_FOUND));
                ...
                verify(auditEmitterRegistry, org.mockito.Mockito.never()).register(any());
        }

        @Test
        void listAudits_returnsPagedResults() throws Exception {
                UUID auditId = UUID.randomUUID();
                Page<AuditResponse> page = new PageImpl<>(List.of(sampleAuditResponse(auditId)), PageRequest.of(0, 20),
                                1);
