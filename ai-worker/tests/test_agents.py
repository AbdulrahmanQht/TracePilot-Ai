...
def test_parse_and_isolate_trace_success():
    ...
    # Tell the fake LLM exactly what string to return when called
    ...
    # Call your extraction function with the fake LLM
    evidence, claims, screening = parse_and_isolate_trace(
        "some raw trace text...", mock_llm
    )

    # Verify the extraction models mapped the data correctly
...
def test_parse_and_isolate_trace_handles_bad_json():
    ...
    evidence, claims, screening = parse_and_isolate_trace("trace text", mock_llm)
